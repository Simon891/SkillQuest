...
df['description.text'].fillna('', inplace=True)

# Convert non-string values to strings
df['description.text'] = df['description.text'].astype(str)

# Tokenize the text data
tokenizer = Tokenizer(num_words=max_words, lower=True, oov_token='<OOV>')
tokenizer.fit_on_texts(df['description.text'].values)

# Define the maximum length of the sequences
max_len = 200

# Convert the text data to sequences of integers
sequences = tokenizer.texts_to_sequences(df['description.text'].values)
...
model.fit([X_train, X_train], y_train, batch_size=32, epochs=20, validation_data=([X_val, X_val], y_val))

# Generate a new job listing
new_job_title = "Systemutveckling"
new_job_text = "Vi söker en skicklig utvecklare till vårt team. Den idealiska kandidaten bör ha erfarenhet av utveckling, programmering och systemdesign."
